...
const { Post, User, Comment } = require('../models');  // Import the models (Post, User, Comment).
...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json(err).json({ message: "return an error if there's a server error from Serialize data" });  // Return an error if there's a server error
  }
});
